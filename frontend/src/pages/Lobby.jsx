...
export default function Lobby() {
    ...
    return (
        <div
            className="min-h-screen text-slate-100 px-4 py-6 md:py-8 flex flex-col justify-center"
            style={
                bgSlug
                    ? {
        ...
        >
            <div className="max-w-3xl mx-auto w-full space-y-6 md:space-y-8 animate-slide-up">
                {/* Header Section */}
                <header className="flex flex-col items-center text-center gap-3 md:gap-4 pb-6 md:pb-8 border-b border-white/5">
                    <div>
                        <h1 className="text-[10px] md:text-sm font-black text-accent tracking-[0.3em] md:tracking-[0.4em] uppercase mb-2">Pre-Auction Briefing</h1>
                        <div className="flex flex-col items-center gap-3">
                            <div className="flex items-center gap-3">
                                <span className="w-2 md:w-3 h-2 md:h-3 rounded-full bg-rose-500 animate-pulse"></span>
                            ...
                            <p className="text-[9px] md:text-[10px] text-slate-500 font-bold uppercase tracking-[0.1em] md:tracking-[0.2em]">
                                Waiting for franchise owners to assemble
                            </p>
                            <div className="flex flex-wrap items-center justify-center gap-2">
                                <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-white">
                                    {roomVisibility === "public" ? "Public Room" : "Private Room"}
                                </span>
                                <span className={`rounded-full border px-3 py-1 text-[9px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.2em] ${
                                    isCreator
                ...
                <main className="space-y-6">
                    ...
                    <section className="glass-card p-6 md:p-8 space-y-6 md:space-y-8 relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-8 text-6xl md:text-[80px] font-black italic text-white/5 select-none pointer-events-none uppercase tracking-tighter">
                            LOBBY
                        </div>

                        <div className="relative z-10 space-y-6">
                            <div className="flex items-center justify-between">
                                <span className="text-[10px] md:text-xs font-black uppercase tracking-widest italic text-white">Confirmed Participants</span>
                                <span className="text-base md:text-lg font-black text-accent italic tracking-tighter">
                                    {players.length} <span className="text-[9px] md:text-[10px] text-slate-500 tracking-widest uppercase not-italic ml-1">Joined</span>
                                </span>
                            </div>

                            <div className="grid gap-3">
                                {players.map((p, i) => (
                                    <div key={i} className="flex justify-between items-center bg-white/5 border border-white/5 p-3 md:p-4 rounded-xl hover:bg-white/10 transition">
                                        <div className="flex flex-col">
                                            <span className="text-white font-bold text-xs md:text-sm uppercase italic tracking-tight">
                                                {p.username} {p.username === username && <span className="text-[9px] md:text-[10px] text-accent ml-2">(YOU)</span>}
                                            </span>
                                        ...
                                        <div className="flex items-center gap-2 md:gap-4">
                                            {p.isCreator && (
                                                <span className="rounded-full border border-accent/20 bg-accent/10 px-2 py-0.5 md:py-1 text-[8px] md:text-[9px] font-black uppercase tracking-[0.1em] md:tracking-[0.2em] text-accent">
                                                    Host
                                                </span>
                                            )}
                                            <span className="text-[10px] md:text-xs font-black text-slate-400 uppercase italic tracking-wide break-words max-w-[80px] md:max-w-none text-right">{p.team || "Independent"}</span>
                                            <div className="w-1.5 md:w-2 h-1.5 md:h-2 rounded-full bg-emerald-500 shrink-0"></div>
                                        </div>
                                    </div>
                                ))}
                                {players.length === 0 && (
                                    <div className="py-12 text-center italic text-slate-600 font-medium tracking-widest uppercase text-[10px] md:text-xs">
                        ...
                        <div className="relative z-10 pt-4">
                            <button
                                className="primary-btn w-full !py-4 !rounded-xl text-xs md:text-sm font-black tracking-[0.1em] md:tracking-[0.2em] uppercase italic disabled:opacity-30 disabled:grayscale transition-all"
                            ...
                            {players.length < 2 && (
                                <p className="mt-4 text-[9px] md:text-[10px] text-slate-500 font-bold uppercase tracking-widest text-center italic">
                    ...
                    <footer className="pt-4 flex justify-center">
                        <button 
                            className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-slate-600 hover:text-white transition-colors flex items-center gap-2"
                            onClick={() => {
                                leaveRoom();
                                navigate("/");
                            }}
